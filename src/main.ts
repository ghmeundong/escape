...
import { bindEpisodeSelection, bindScreenFlow } from "./game/ui";
import "./style.css";
...
developerTestGroup.append(
  ...
  waypointDisplayToggleLabel,
  waypointEditToggleLabel,
);
...
function logMatryoshkaUserWaypoints(): void {
  ...
  console.log(
    "[Matryoshka waypoints] Copy this JSON:",
...
function addMatryoshkaWaypointFromAim(): void {
  ...
  const hit = parkingGroundRaycaster
    .intersectObject(parkingLotRoot, true)
    .find(
      (intersection) => intersection.point.y >= parkingBounds!.min.y - 0.25,
    );
  ...
  if (
    matryoshkaUserWaypoints.some(
      (existing) => existing.distanceToSquared(waypoint) < 0.25,
    )
  )
    return;
...
function findNearestMatryoshkaWaypoint(
  position: THREE.Vector3,
  ...
  for (const waypoint of waypoints) {
    ...
    if (
      ...
      isMatryoshkaPathClear(position, waypoint)
    ) {
      nearest = waypoint;
...
function findMatryoshkaRoute(
  ...
  if (Math.abs(end.y - start.y) <= 2.5 && isMatryoshkaPathClear(start, end))
    return [end.clone()];
  ...
  while (open.length > 0) {
    ...
    for (const neighbor of waypoints) {
      if (
        ...
        !isMatryoshkaPathClear(current, neighbor)
      )
        continue;
      const nextDistance =
...
function getMatryoshkaRouteDistance(
  start: THREE.Vector3,
  route: THREE.Vector3[],
): number {
  return route.reduce(
...
function _findMatryoshkaNonOverlappingRoute(
  ...
  candidateRoutes.sort((first, second) => {
    ...
    return (
      overlapDifference ||
      getMatryoshkaRouteDistance(start, first) -
        getMatryoshkaRouteDistance(start, second)
    );
  });
...
waypointDisplaySetting.addEventListener("change", () => {
  matryoshkaWaypointMarkers.forEach((marker) => {
    marker.visible = waypointDisplaySetting.checked;
  });
  matryoshkaRecoveryWaypointMarkers.forEach((marker) => {
    marker.visible = waypointDisplaySetting.checked;
  });
});
...
function placeWeaponPickupWhenReady(): void {
  ...
  for (let attempt = 0; attempt < 40; attempt += 1) {
    ...
    if (
      parkingBounds &&
      (x < parkingBounds.min.x + 1 ||
        x > parkingBounds.max.x - 1 ||
        z < parkingBounds.min.z + 1 ||
        z > parkingBounds.max.z - 1)
    )
      continue;
    if (overlapsMatryoshkaVehicleObstacle(x, z, matryoshkaObstaclePadding))
      continue;
...
function finishEpisodeEntryLoading(): void {
  requestAnimationFrame(() => {
    ...
    requestAnimationFrame(() => {
      ...
      requestAnimationFrame(() => {
        episodeEntryLoading = false;
        range.classList.remove("is-enter-loading");
        _hideLoadingScreen(true);
      });
    });
  });
}

function enterEpisode(episodeId: EpisodeId): void {
...
settingsClose.addEventListener("click", () => {
  if (window.electronAPI && !startScreen.classList.contains("is-visible"))
    enterGame();
  else closeMenu();
});
...
const parkingPreviewRenderer = new THREE.WebGLRenderer({
  canvas: parkingPreviewCanvas,
  antialias: true,
  alpha: false,
});
...
let lastParkingPreviewFrameAt = 0;
parkingPreviewRenderer.setAnimationLoop(() => {
  if (!episodeScreen.classList.contains("is-visible")) return;
  const now = performance.now();
  const previewFrameInterval = parkingLotEpisodeButton.matches(":hover")
    ? 16
    : 100;
  if (now - lastParkingPreviewFrameAt < previewFrameInterval) return;
  lastParkingPreviewFrameAt = now;
  const width = parkingPreviewCanvas.clientWidth;
  const height = parkingPreviewCanvas.clientHeight;
  if (!width || !height) return;
  parkingPreviewRenderer.setSize(
...
const storePreviewRenderer = new THREE.WebGLRenderer({
...
let lastStorePreviewFrameAt = 0;
storePreviewRenderer.setAnimationLoop(() => {
  if (!episodeScreen.classList.contains("is-visible")) return;
  const now = performance.now();
  const previewFrameInterval = storeEpisodeButton.matches(":hover") ? 16 : 100;
  if (now - lastStorePreviewFrameAt < previewFrameInterval) return;
  lastStorePreviewFrameAt = now;
  const width = storePreviewCanvas.clientWidth;
  const height = storePreviewCanvas.clientHeight;
  if (!width || !height) return;
  storePreviewRenderer.setSize(
    Math.max(160, Math.floor(width * 0.75)),
    Math.max(160, Math.floor(height * 0.75)),
    false,
  );
...
matryoshkaPreviewScene.add(
  new THREE.HemisphereLight("#d9e0df", "#111519", 2.2),
);
...
const matryoshkaPreviewRenderer = new THREE.WebGLRenderer({
  canvas: matryoshkaPreviewCanvas,
  antialias: true,
  alpha: false,
});
matryoshkaPreviewRenderer.setPixelRatio(1);
const matryoshkaPreviewGroup = new THREE.Group();
matryoshkaPreviewScene.add(matryoshkaPreviewGroup);
let lastMatryoshkaPreviewFrameAt = 0;
void matryoshkaModelReady
  .then((model) => {
...
matryoshkaPreviewRenderer.setAnimationLoop(() => {
  if (!startScreen.classList.contains("is-visible")) return;
  const now = performance.now();
  if (now - lastMatryoshkaPreviewFrameAt < 33) return;
  lastMatryoshkaPreviewFrameAt = now;
  ...
  matryoshkaPreviewRenderer.render(
    matryoshkaPreviewScene,
    matryoshkaPreviewCamera,
  );
});
